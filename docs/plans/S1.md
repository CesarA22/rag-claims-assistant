...
The S1 plan described a pipeline that later sessions would fill in behind
the four Protocols. The tree as of S4 still matches that contract on the
HTTP shape, and diverges on four things the plan (and AGENTS.md) listed as
arriving sooner than they did:

- `ask.py` still has no `redact()` step. AGENTS.md lists it as step 7 of
  the pipeline; ingest-time redaction and the claims query layer exist, but
  the output scrubber is not wired into the turn.
- `llm` in `ask()` is still the raw provider. There is no `resilient.py`
  decorator, no timeout budget, no retry, no circuit breaker.
- The tool-planning model call is still absent. `ClaimsTool` exists as of
  S4 and is not composed into `ask.py` or `main.py`.
- `create_app` still defaults to `InMemoryConversationRepository`. Postgres
  holds chunks; conversation history is an in-memory dict.

Those are deferred, not forgotten. The envelope, citation validator, and
import boundary (`openai` only in `app/llm/openai_provider.py`) are the
S1 invariants that still hold.

A real POST to `/conversations/c-1/messages` against `uvicorn` with
`LLM_PROVIDER=fake` returned `outcome=answered`, `answer` containing
"12 meses", and `citations[0]` with `document_code=CG-AUTO-2024`,
`section=2.1 Vigência`, `version=3.2`, `effective_date=2024-01-01`.
