...
(Opus 5) for S8, S9 and S10 — the front end and the API changes it forced,
driving the browser for the chaos run, the eval harness, and the container.
...
| 9 |2026-09-04| S9 — the eval harness, EVALS.md, README verified from a clean clone, architecture diagram | Thresholds in their own commit before the harness could produce a result, so "fixed beforehand" is `git log --follow evals/thresholds.yaml` rather than a sentence to be believed; `deterministic_pass_rate: 0.83` registered rather than 1.00, because gs-007 was going to fail for a known reason and registering 6/6 leaves room to relax it afterwards. Assertion scopes split per check — `must_not_contain` on the answer only, `forbidden_strings` and `must_not_match` on the whole envelope; one asks whether it said a wrong thing, the other whether anything private got out. `must_cite` matches `section` as well as `document_code`, since claims evidence is `document_code="claims.db"` with the call string in `section`. One conversation and one `client_message_id` per case per run, or run 2 replays run 1 at zero cost and history contaminates every later question. The judge sees the answer, the full cited chunks and the criterion, never `resposta_esperada`; it runs in the eval process with its own provider so its spend never lands in a QuestionBudget. EVALS.md generated from committed raw results, never hand-edited. The architecture diagram draws the pipeline that exists | The error-leakage gate as written scanned every response for `"gpt-5"` — which fails all thirty successful live responses, because `meta.model` is in the envelope on purpose and T-02 asserts it. `00-architecture.mdc` scopes that prohibition to **errors**. Fixed before any live run existed, and the threshold did not move: a problem+json body allows none of the three, a success envelope allows the model name in `meta.model` and nowhere else, so a model name reaching `answer` or a snippet is still caught. Stricter than what it replaced. Also caught myself writing a false finding: the fresh-clone transcript claimed `pyyaml` was only reaching the venv transitively through `uvicorn[standard]`, and it is declared in `pyproject.toml` — removed before it reached EVALS.md, because a fabricated defect in the one file whose value is honesty is worse than no finding at all. And `git add -A` swept the S10 plan into S9d's commit; split back out so S9d's diff is what S9d did | **The live tier could not run: the API key has no credits** (`429 insufficient_quota`, `credit_balance_exhausted`), captured verbatim to `evals/results/tier2-blocked.txt` and printed at the top of EVALS.md. So: answer quality, cost, the cached/uncached split, latency and the four judge cases are **not measured**, and the file says so before it says anything else. What is measured is real, because the graded outcomes are decided in `grounding.py` after the model and overrule it: citation validity 30/30, PII 30/30 (checked on every response, not only gs-010's), error leakage 30/30, boundary 9/10, tier 0 recall@5 = 9/9 = 100% against a registered 0.88. THE FINDING, and it is provider-independent: gs-008 returns `needs_clarification`, not `refused`, because `judge()` checks ambiguity before sufficiency and gs-008's evidence spans three products. Measured both ways — a model that attempts an answer gets a clarification, one that refuses on its own gets a refusal — so R-02's deterministic backstop, the whole reason T-16 exists, never executes for that question. Left unfixed on purpose: a boundary finding gets its own commit and its own test. README followed literally from a clone into an empty directory against a **new empty database**, so `alembic upgrade head` ran from nothing: 13 documents / 220 chunks / 13 tables / 1 footnote, the three documented questions returning answered / needs_clarification with `['Auto','Residencial']` / refused, 76 + 10 + 2 tests green, `npm run build` clean. `judge_run.py` verified to fail honestly against the exhausted key — writes no verdicts rather than inventing one |
| 10 |2026-09-04| S10 — the whole application in one `docker compose up`: multi-stage image, the API mounted under a shell that also serves the built client, idempotent boot, D-03 closed | One Dockerfile and one service, not the two S8 estimated: `mount_client()` puts `create_app()` at `/api` and `web/dist` at `/`, which reproduces the Vite proxy's rewrite exactly, moves no route, and keeps client and API on one origin so S8's refusal to add CORS is now true in a deployment instead of only behind a dev proxy. Boot is migrate → ingest-if-empty → serve, because a fresh volume is an empty index and an empty index refuses every question — correct behaviour that looks like a broken product. `--no-embed` and `RETRIEVER_ARM=lexical` fixed rather than overridable, since the container has no vectors to read. Non-root uid, no apt layer, `.dockerignore` keeping `.env` and the 303 MB venv out, `.gitattributes` pinning the entrypoint to LF. T-43/T-44 in `tests/test_container_app.py`; D-03 `partial → done` | **Eight things in my own S10 plan, which I had approved, turned out to be wrong, and the first one would have shipped a broken demo that looked like a working one.** The plan reassured me that "a mounted ASGI sub-app keeps its own middleware and exception handlers" — true, and the wrong reassurance: Starlette never routes the *lifespan* scope into a mount, so `create_app()`'s startup hook would not have run, `state.retriever` would have stayed the two-chunk `InMemoryRetriever`, and `/healthz` would have said `ok` while every answer came from fixtures. Also rejected: `libgl1`/`libglib2.0-0` as a required apt layer (those are opencv's, not pymupdf's); `uvicorn app.main:container_app`, which builds a second app and a second SQLAlchemy engine at import in every process including the test suite, replaced by `--factory`; the entrypoint's guard as drafted, `if [ "$(chunk_count)" = "0" ]`, where a crash inside a condition is invisible to `set -e` and would skip the ingest and serve an empty index — the exact outcome the guard exists to prevent, reached through the guard; a healthcheck with no `start_period`, which reports unhealthy for the whole first ingest; and a compose block missing the three `CHAOS_*` rates, which the plan's own gate needs — "all seven states reachable", and two of the seven only exist when the provider fails. Separately, the shell's default `/docs`, `/redoc` and `/openapi.json` are registered before the mounts and would have shadowed the client with an empty schema. A parallel audit of the plan raised 33 findings and 31 did not survive adversarial verification; the two that did were both documentation | Ran the failure before writing the fix: a five-line script mounting a child with a startup hook printed `child startup fired? []`, then printed `['startup','shutdown']` once the shell drove `api.router.lifespan_context`. pymupdf checked by *using* it in `python:3.12-slim` with no apt layer — `get_text("dict")` and `find_tables()` over the real thirteen PDFs — not by importing it. **The cold-volume run is the only one that counts**, so I backed up the dev volume first (`pg_dump`, 220 chunks / 220 embeddings, unrepeatable — the key has no credits) and ran every container test under a separate compose project so `down -v` could never reach it. Then `git clone` into an empty directory at the final commit, no `.env`, no venv, no `node_modules`: `--no-cache` build 29 s, boot 15 s, `alembic` upgrading from nothing, 13 documents / 220 chunks / 13 tables / 1 footnote / 1 PII chunk. 11/11 checks — `/api/healthz` ok·sql·fake·db-reachable, the client at `/`, `/api/docs` alive while the shell's `/docs` falls through to the client 404, gs-001 answered citing CG-AUTO-2024 §2.1 p2 v3.2, **gs-009 returning `['Auto','Residencial']`, which is the check that catches the lifespan bug** because that gate cannot fire against the in-memory fixture, gs-010 refused with no CPF anywhere in the envelope, idempotent replay to the same `message_id`, history through the mount. Second boot: `index already holds 220 chunks; skipping ingest`. With `LLM_PROVIDER=chaos CHAOS_500_RATE=1.0`: degraded at 200 with `reason=provider_degraded` and 5 citations, failed at 503 `application/problem+json` with a trace id and no model name, prompt or traceback — which also proves the RFC 9457 handlers survive the mount. In the browser at `http://localhost:8000`, the vidros question renders **PRECISA DE ESCLARECIMENTO** with both chips, no console errors. `pytest --disable-socket -q` 79 passed / 10 deselected (the 76 unchanged), `pytest -m db` 10 passed, register ok. One thing I got wrong on the way: I read the first chaos response as "not degraded" because I looked at `outcome` instead of `meta.degraded`, and said so before re-reading the envelope |
<!--
...
### 4. Fifteen lines that would have served fixtures and reported themselves healthy

**Suggested:** the S10 plan's `create_container_app()`, mounting the API at
`/api` inside a shell, with the reassurance that *"a mounted ASGI sub-app keeps
its own middleware and exception handlers"* — followed by three bullets on why
that made the change safe.

**Rejected because:** the sentence is true and it is not the risk. Middleware and
exception handlers do survive a mount; the **lifespan scope does not reach one at
all**. Starlette's router handles `lifespan` itself and returns. `create_app()`
opens the asyncpg pool in a startup hook — it has to, because `create_pool` is
async and `create_app` is not — so the plan as written would have left
`state.retriever` as the two-chunk `InMemoryRetriever` in the container.

What makes this the worst kind of defect rather than an ordinary one: **nothing
looks wrong.** `/healthz` returns `ok`. The database is reachable. The corpus is
indexed and sitting in Postgres. The README's first demo question still answers
and still cites CG-AUTO §2.1, because those exact strings are in the in-memory
fixture. An evaluator would have had to ask a question the fixture cannot serve
before anything gave way — and the plan's own acceptance test was the question
the fixture *can* serve.

**How I caught it:** by not believing the paragraph. Five lines — mount a child
with a startup hook, request through the parent — printed:
...
The fix forwards the child's lifespan from the shell, and the acceptance test
moved to gs-009: `InMemoryRetriever` spans one product, so
`clarification_options: ['Auto', 'Residencial']` coming back from the container
is mechanical proof that the real hybrid index is behind the mount. T-44 pins the
forwarding directly.

**The general lesson, which is the reason this is written down:** the plan was
confident, specific, and cited the right mechanism for the wrong property. Seven
more of its claims turned out to be wrong the same way — an apt layer that is
opencv's, a guard that `set -e` cannot see through, a healthcheck with no start
period. Every one of them was found by running the thing rather than reading
about it, and none of them by reading the plan again more carefully.
